...
function setup() {
  ...
  deveshi = new Hexagon(200, 200, 40);
  // deveshi = Bodies.circle(200, 200, 40);
  World.add(world, deveshi);
  slinj = new slingshot(this.deveshi.body, { x: 250, y: 160 });
}
function draw() {
  ...
  slinj.display();
  imageMode(CENTER);
  // image(hexagonIMG, deveshi.position.x, deveshi.position.y, 40, 40);
  // deveshi.position.x = mouseX;
  // deveshi.position.y = mouseY;

  drawSprites();
}
function mouseDragged() {
  Matter.Body.setPosition(deveshi.body, { x: mouseX, y: mouseY });
}
